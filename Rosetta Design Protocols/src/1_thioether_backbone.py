...
import pyrosetta
pyrosetta.init('-ex1 -ex2 -ex3 -ex4 -score:symmetric_gly_tables true -set_weights atom_pair_constraint 1 dihedral_constraint 1 angle_constraint 1 mm_twist 0 -out:file:output_virtual false -mute all')

class cyc_pep_predict_thioether:
  ...
  def pack_thioether_rotamer(self):
  ...
  def output_pose(self):
    if self.closure_check():
      ...
      self.pose.center()
      if final_score < -2:      
	self.pose.dump_pdb('/thioether_'+sys.argv[1]+'mers/' + 'thioether_cycle_' + str(self.res_length) + 'mer_' + str(final_score) + '.pdb')
  def run(self):
    for i in range(n_struct):
      self.randomize_all_phipsi()
      self.pack_thioether_rotamer()
...
cyclic_peptide = cyc_pep_predict_thioether(res_length, n_struct)
cyclic_peptide.run()
